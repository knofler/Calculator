...
switch(strtolower(fetch('format'))){
    ...
    //plain text
        ...
        case 'xml':
          ...
          array_walk($out_deduction,create_function('$i,$k','global $xml;$xml->addChild($k,$i);'));
          echo $xml->asXML();
          break;
        case 'jsonp':
            header('Content-Type:application/javascript;charset=UTF');
            echo fetch('callback'),'(',json_encode($out_deduction),');';
            break;
        default:
            header('Content-Type:application/json; charset=UTF-8');
            echo json_encode($out_deduction);
            break;
}
